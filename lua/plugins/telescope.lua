...
return {
    {
        ...
        keys = function()
            ...
            return {
                { "<leader>sf", function() builtin.find_files({ hidden = true }) end },
                { "<leader>sh", function() builtin.help_tags() end },
                { "<leader>sg", function() builtin.live_grep() end },
                { "<leader>sw", function() builtin.grep_string(themes.get_dropdown()) end },
                { "<leader>/", function() builtin.current_buffer_fuzzy_find(themes.get_dropdown({ previewer = false })) end },
            }
        end,
        config = function()
